...
class GenericXLSXAdapter(SourceAdapter):
    """
    Generic Excel XLSX Source Adapter.
    Processes Excel files into Canonical Records with Quarantine isolation.
    """
    ...
    def profile(self, file_path: str, sheet_name: Optional[str] = None) -> DataProfile:
        return SourceProfiler.profile_file(file_path, sheet_name=sheet_name)
            ...
            for src_col, canon_field in col_map.items():
                if src_col in df.columns:
                    ...
                    if pd.notnull(val):
                        ...
                        mapped_row[canon_field] = val
                    else:
                        mapped_row[canon_field] = None

            mapped_row["source_file"] = filename
            mapped_row["source_row"] = source_row_num
            mapped_row["source_system"] = "GENERIC_XLSX"
