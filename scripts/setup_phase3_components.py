...
# 1. Types
# ==========================================
files['apps/frontend/src/types/dashboard.ts'] = """export type DataStatus = 'OK' | 'DATA_INCOMPLETE' | 'SYNTHETIC';
...
files['apps/frontend/src/types/facts.ts'] = """export interface DailyFactItem {
  business_date: string;
  sales: number | null;
  guests: number | null;
  avg_check: number | null;
...
files['apps/frontend/src/types/evidence.ts'] = """export interface EvidenceDetail {
  evidence_id: string;
...
# 2. Utils
# ==========================================
files['apps/frontend/src/utils/formatters.ts'] = """export function formatWon(value: number | null | undefined, kpiStatus?: string): string {
...
files['apps/frontend/src/utils/ruleDisplay.ts'] = """export interface RuleMeta {
  name: string;
  description: string;
...
export const RULE_REGISTRY: Record<string, RuleMeta> = {
  'R-LAB-01': {
    name: '인건비율 기준 초과',
    description: '일일 인건비율이 적정 관리 기준(33%)을 초과했습니다.',
    category: 'LABOR',
  },
  'R-INV-01': {
    name: '재고 차이 확인 필요',
    description: '실사 재고와 이론 재고 간 차이가 -3.0kg 이상 발생하여 실사 확인이 필요합니다.',
    category: 'INVENTORY',
  },
  'R-FC-01': {
    name: '식재료 원가율 기준 초과',
    description: '일일 식재료 원가율이 적정 관리 기준(38%)을 초과했습니다.',
    category: 'COST',
  },
  'R-FC-01-PERIOD': {
    name: '식재료 원가율 지속 상승',
    description: '최근 7일 이상 연속으로 원가율이 39% 이상 유지되고 있습니다.',
    category: 'COST',
  },
  'R-WST-01': {
    name: '폐기율 기준 초과',
    description: '판매량 대비 식재료 폐기율이 기준(2.0%)을 초과했습니다.',
    category: 'INVENTORY',
  },
  'R-CUS-01': {
    name: '고객경험 확인 필요',
    description: '고객 불만 건수(3건 이상) 또는 평점(4.2점 이하) 이상이 감지되었습니다.',
    category: 'CUSTOMER',
  },
  'R-PRO-01': {
    name: '매출 증가 대비 수익성 악화',
    description: '매출이 15% 이상 증가했으나 공헌이익률이 3%p 이상 하락했습니다.',
    category: 'PROFIT',
  },
  'R-DQ-01': {
    name: '필수 데이터 누락',
...
files['apps/frontend/src/utils/status.ts'] = """export function getSeverityStyle(severity: 'CRITICAL' | 'HIGH' | 'MEDIUM') {
  switch (severity) {
    case 'CRITICAL':
      return {
        badgeBg: '#450a0a',
...
# ==========================================
# 7. Components: Trend Charts (Recharts)
# ==========================================
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Chart B: Labor Ratio & Food Cost Ratio */}
        <div style={{ height: '220px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
...
# ==========================================
# 8. Components: Summary & Evidence Drawer
# ==========================================
    ...
    <div style={{ minHeight: '100vh', backgroundColor: '#090d16', color: '#f8fafc' }}>
      ...
      <main style={{ maxWidth: '1440px', margin: '0 auto', padding: '24px 20px' }}>
        {dailyLoading ? (
          ...
          <div>
            {/* DATA_INCOMPLETE Warning Banner */}
            ...
            {/* Alerts & Operational Panels Grid */}
            <div
              style={{
                display: 'grid',
                gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))',
...
print(f"Successfully generated {len(files)} frontend components, types, and hooks.")
